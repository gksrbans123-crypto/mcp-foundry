 ...
 *
 * Ported verbatim from apps/runtime-host/src/egress/ip-range-check.ts (task
 * #4) — apps/worker needs the identical SSRF gate for its probe/compliance
 * checks but cannot import runtime-host's internals (its package.json only
 * exports its process-bootstrap entrypoint, which has import-time side
 * effects). See apps/worker's completion report for the consolidation
 * follow-up recommendation (extract into a shared package).
 */
